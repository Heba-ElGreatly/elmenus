...
insert into orders (user_id,ordered,created_on,total) values(
(select user_id from user where email='test@gmail.com'),
0,
null,
0
);
...
values (
2,
(select item_id from item where item_name = 'burger'),
(select order_id from orders where ordered=0 and user_id = (select user_id from user where email='test@gmail.com'))
);

insert into cart_item (item_quantity,item_id,order_id)
values (
2,
(select item_id from item where item_name = 'fish'),
(select order_id from orders where ordered=0 and user_id = (select user_id from user where email='test@gmail.com'))
);